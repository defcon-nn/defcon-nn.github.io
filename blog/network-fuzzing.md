# Цикл практической-ориентированных статей "Фаззинг сетевых компонент".
...
- chat-cli: консольный клиент для этого чат-сервера (Замечание: в репозитории https://gitlab.com/DC_ZN/telyogram лежит клиент под Андроид)
...
Замечание. Для простоты реализации модельного сервера в нем не сделан модуль конфигурации, значения по умолчанию удобны для размещения сервера в докер-контейнере. Адрес и порт можно поменять, только отредактировав исходный код chatd, который находится в файле `server.c`.  Если вы работаете с chatd в какой-то недоверенной сети (например, публичный Wifi) без использования механизма изоляции (виртуальная машина, контейнер), то запуск сервисов на публичном интерфейсе открывает вас к угрозам безопасности.

Кроме того, в дальнейшем мы планируем проводить фаззинг-тестирование этого сервера, и опять-таки без применения изоляции это может привести к проблемам - в общем-то к выполнению неожиданного кода с правами текущего пользователя. Для защиты от повреждения окружения текущего пользователя нужно использовать какую-то систему изоляции. В данном случае достаточно создать отдельного ограниченного пользователя (не имеющего доступ на запись, за исключением явных разрешений). На Debian-подобных дистрибутивах для этого используется команда `adduser`:
```
...
Как видим, чат более-менее работает. Заметны баги с отправкой и доставкой сообщений - иногда доставляются только вместе со следующим или при отправке ответа; скорее всего баг в клиенте, он вообще был написан за пару часов - насколько помню, в джавовом клиенте под Андроид, telyogram, этот баг не проявляется.
